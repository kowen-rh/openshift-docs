...
where:
+
`storage: 200Gi`:: The amount of storage available to the PVC. Adjust this value for your requirements.
+
[NOTE]
====
...
where:
+
`storage: 100Gi`:: The amount of storage available to the PV. Adjust this value for your requirements.
`<example_master_node>`:: Replace this value with the master node to attach this PV to.
+
[TIP]
====
...
where:
+
`storage: 10Gi`:: The amount of storage available to the PVC. Adjust this value for your requirements.
...
where:
+
`schedule: "20 4 * * *"`:: The `CronTab` schedule for recurring backups. Adjust this value for your needs.
+
...
where:
+
`retentionType: RetentionNumber`:: The retention type. Defaults to `RetentionNumber` if unspecified.
`maxNumberOfBackups: 5`:: The maximum number of backups to retain. Adjust this value for your needs. Defaults to 15 backups if unspecified.
+
[WARNING]
====
...
where:
+
`maxSizeOfBackupsGb: 20`:: The maximum file size of the retained backups in gigabytes. Adjust this value for your needs. Defaults to 10 GB if unspecified.
+
[WARNING]
====
