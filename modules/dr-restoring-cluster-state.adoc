...
For high availability (HA) clusters, a three node HA cluster requires you to shutdown etcd on two hosts to avoid a cluster split. On four and five node HA clusters, you must shut down three hosts. Quorum requires a simple majority of nodes. The minimum number of nodes required for quorum on a three node HA cluster is two. On four and five node HA clusters, the minimum number of nodes required for quorum is three. If you start a new cluster from backup on your recovery host, the other etcd members may still be able to form quorum and continue service.
...
. SSH into the recovery host and run the following command:
+
[source,terminal]
----
...
. Once the API responds, turn off the etcd Operator quorum guard by runnning:
+
[source,terminal]
----
...
. Wait until the control plane recovers by running:
+
[source,terminal]
----
...
. Once recovered, enable the quorum guard by running:
+
[source,terminal]
----
