...
where:
+
`clustername-8qw5l-master-0`:: This is the control plane machine for the offline node, `ip-10-0-131-183.ec2.internal`.
...
where:
+
`clustername-8qw5l-master-0`:: Specify the name of the control plane machine for the offline node.
+
...
where:
+
`clustername-8qw5l-master-3`:: The new machine, `clustername-8qw5l-master-3` is being created and is ready after the phase changes from `Provisioning` to `Running`.
+
