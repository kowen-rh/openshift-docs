...
where:
+
`stopped`:: This output lists the node and the status of the node's machine. If the status is anything other than `running`, then the *machine is not running*.
+
// TODO: xref
...
where:
+
`node.kubernetes.io/unreachable`:: If the node is listed with an `unreachable` taint, then the *node is not ready*.
...
where:
+
`NotReady`:: If the node is listed as `NotReady`, then the *node is not ready*.
...
where:
+
`Error`:: Since this status of this pod is `Error`, then the *etcd pod is crashlooping*.
