...
where:
+
`ip-10-0-131-183.ec2.internal`:: Pass in the name of the unhealthy etcd member that you took note of earlier in this procedure.
+
...
where:
+
`clustername-8qw5l-master-0`:: This is the control plane machine for the unhealthy node, `ip-10-0-131-183.ec2.internal`.
...
where:
+
`clustername-8qw5l-master-0`:: Specify the name of the control plane machine for the unhealthy node.
+
...
where:
+
`clustername-8qw5l-master-3`:: The new machine, `clustername-8qw5l-master-3` is being created and is ready once the phase changes from `Provisioning` to `Running`.
+
...
where:
+
`forceRedeploymentReason`:: The `forceRedeploymentReason` value must be unique, which is why a timestamp is appended.
